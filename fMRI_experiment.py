...
if trigger_mode:
    ...
    for n in range(5): #do several times for debugging or once in real expt
...
for trial, stimulus in enumerate(stimuli_list_shuffled):
    ...
    if (trial != 0) & (trial % 12 == 0): #every 12 trials, at the *start* of the 13th trial. *After* 12 trials are completed. Trial 12 in python is Trial 13 in human 
        ...
        dur_schluckpause = 10 - (start_schluckpause - (triggertime + (cue_dur + stim_dur_main_expt + ISI_time) * trial + ISI_time + 10 * nSchluck)) # will depend on delays upstream. So would be slightly less than 10s
...
#                    writerlog.writerow(["Trial " + str(trial + 1) + "Omission onset", core.getTime(), core.getTime()-trial_start_time])  
    ...
    print("Start of 1st Temp rec: " + str(core.getTime()-triggertime) + " after trigger and " + str(core.getTime() - trial_start_time) + " after trial start")
    writerlog.writerow(["Trial " + str(trial + 1) + "After win flip + start of temp rec", core.getTime(), core.getTime()-triggertime, core.getTime() - trial_start_time])      
    if thermode:    
        # Record temps for 1s
    ...
    print("ISI should be " + str(total_trial_len - (onset_of_ISI - (triggertime + (cue_dur + stim_dur_main_expt + ISI_time) * trial + ISI_time + 10 * nSchluck))) + " s long...")
    ...
    remainingISI = total_trial_len - (end_of_temp_rec - (triggertime + (cue_dur + stim_dur_main_expt + ISI_time) * trial + ISI_time + 10 * nSchluck))

    print("Rest of ISI should be " + str(total_trial_len - (end_of_temp_rec - (triggertime + (cue_dur + stim_dur_main_expt + ISI_time) * trial + ISI_time + 10 * nSchluck))) + "s long...")
    core.wait(remainingISI) #pinned to the trigger start rather than trial start
    print("Trial end: " + str(core.getTime() - trial_start_time))
